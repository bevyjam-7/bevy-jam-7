...
use crate::{AppSystems, PausableSystems, game_consts::{BRIDGE_SECTION_LOCATION, TELEPORTER_PROXIMITY_RADIUS, WITCH_HOUSE_LOCATION}, player::player::Player};
...
pub fn player_transition_between_sections(
    mut transition_query: Query<(&Transform, &mut TranstionBetweenSections), (Without<Player>, Without<Camera>)>,
        ...
        if distance_sq <= sqrt(transtion_section.transition_radius) {
            match transtion_section.next_section {
                MapSection::WitchHouse => {
                    player_transform.translation = WITCH_HOUSE_LOCATION;
                    camera_transform.translation = WITCH_HOUSE_LOCATION;
                },
                MapSection::Bridge => {
                    player_transform.translation = BRIDGE_SECTION_LOCATION;
